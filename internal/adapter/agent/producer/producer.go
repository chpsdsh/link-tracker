...
func NewKafkaProducer(conf config.KafkaConfig, logger *slog.Logger, linkUpdatesChan chan pkg.KafkaLinkUpdate) (*KafkaProducer, error) {
	...
	return &KafkaProducer{producer: producer,
		NotificationTopic: conf.ProcessedNotificationsTopic,
...
func (p *KafkaProducer) SendLinkUpdate(update pkg.ProcessedLinkUpdate, eventID string) error {
	...
	linkUpdate := pkg.KafkaLinkUpdate{Value: bytes, EventID: eventID}
