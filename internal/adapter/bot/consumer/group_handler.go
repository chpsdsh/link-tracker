...
func NewGroupHandler(dlqProducer producer.DlqProducer, tgHandler handler.TelegramBotHandler, inboxRepo InboxRepository, logger *slog.Logger) GroupHandler {
	return GroupHandler{dlqProducer: dlqProducer, TgHandler: tgHandler, logger: logger, InboxRepo: inboxRepo}
}
...
func (h GroupHandler) deduplicateMessage(sess sarama.ConsumerGroupSession, msg *sarama.ConsumerMessage) (string, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), repositoryRequestTimeout)
	defer cancel()
	if err := h.InboxRepo.Save(ctx, eventID); err != nil {
		if errors.Is(err, repository.ErrNotificationAlreadySent) {
			sess.MarkMessage(msg, "")
			sess.Commit()
